...
p <- ggplot(dataset, aes(x=Date, y=Total, fill=Sales)) +  
    geom_bar(stat='identity', position='dodge') +
    theme_minimal() +
    ggtitle("Total Sales by Product Line and Sales Class")

# Adjust the breaks of the x-axis to display fewer labels
p + scale_x_discrete(breaks = seq(1, length(unique(dataset$Date)), by =  2))
